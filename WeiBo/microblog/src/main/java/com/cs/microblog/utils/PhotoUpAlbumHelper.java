...
public class PhotoUpAlbumHelper extends AsyncTask<Boolean, Object, ArrayList<PhotoBucket>> {
    ...
    public Photo getPhotoByName(File dir) {
        ...
        Cursor cur = mContentRecolver.query(Media.EXTERNAL_CONTENT_URI, columns, Media.DISPLAY_NAME+"=?", new String[]{name}, Media.DATE_MODIFIED + " desc");
        if (cur == null) {
            return null;
        }
        if (cur.moveToFirst()) {
